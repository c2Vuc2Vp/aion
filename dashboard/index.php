<?php
  ...
  if (!isset($_SESSION['id']) && isset($_COOKIE['yumeres']) && !empty($_COOKIE['remember'])) {
    ...
    $split = mb_split("==", $_COOKIE['yumeres']);
    ...
    if ($cookie == $usercookie) {
      ...
      setcookie("yumeres", $usermail."==".sha1($usermail."anotherlevelhackey"), time() + 60*60*24*7, '/');

      $_SESSION["auth"] = $user;

      $cookie = $_COOKIE['yumeres'];
  ...
  if(!empty($_POST) && !empty($_POST["mail"]) && !empty($_POST["password"])){
    ...
    if(isset($_REQUEST['login'])) {
      ...
      $sel = $pdo->query("SELECT * FROM clients WHERE mail = '$mail'");

      $posted = $sel->fetch();

      $data = !empty($posted['password']) ? $posted['password'] : NULL;
      ...
      if(password_verify($pass, $data)){
        ...
        setcookie("yumeres", $_POST["mail"]."==".sha1($_POST["mail"]."anotherlevelhackey"), time() + 60*60*24*7, '/');
